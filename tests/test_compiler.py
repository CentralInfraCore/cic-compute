...
def make_project_config(version="0.1.0", component="compute/"):
    return {
...
def test_validate_release_invalid_version_increment(mocker):
    ...
    def git_cmd(cmd):
        if "status" in cmd: return ""
        if "rev-parse" in cmd: return "compute/releases/v0.9.0"
        if "tag" in cmd: return "compute/@v0.1.0"
        return ""
    mocker.patch("tools.compiler.run_git_command", side_effect=git_cmd)
    with pytest.raises(SystemExit) as e:
        compiler.validate_release_prerequisites()
...
def test_validate_release_first_release(mocker):
    ...
    def git_cmd(cmd):
        if "status" in cmd: return ""
        if "rev-parse" in cmd: return "compute/releases/v0.2.1"
